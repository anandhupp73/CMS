Construction Management System

A simple Construction Management System built with Django, HTML, Tailwind CSS, and SQLite3. Designed to manage construction projects with role-based access for Admins, Project Managers, Supervisors, Contractors, and Accountants.

Features

Role-Based Access: Admin, Project Manager, Supervisor, Contractor, Accountant

Project Management: Create and track construction projects

Task & Assignment Management: Assign tasks and monitor progress

Basic Reporting: Track project status and finances

Responsive UI: Built with Tailwind CSS

Tech Stack

Backend: Django

Frontend: HTML + Tailwind CSS

Database: SQLite3

Authentication & Authorization: Django built-in user model with role-based permissions
